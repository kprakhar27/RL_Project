...
import utils.replay as replay_lib
import utils.types as types_lib
from utils import normalizer
from utils import transforms
from utils import nonlinear_bellman
from utils import base
from utils import distributed
from utils import bandit
from utils.curiosity import EpisodicBonusModule, RndLifeLongBonusModule
from utils.networks.value import Agent57NetworkInputs
...
class Learner(types_lib.Learner):
    ...
    def _update_q_network(self, transitions: Agent57Transition, weights: np.ndarray) -> np.ndarray:
        weights = torch.from_numpy(weights).to(device=self._device, dtype=torch.float32)  # [B,]
        ...
        if burn_transitions is not None:
            ...
            ext_hidden_s = tuple(s.clone().to(device=self._device) for s in init_ext_hidden_s)
            target_ext_hidden_s = tuple(s.clone().to(device=self._device) for s in init_ext_hidden_s)
            # Make copy of hidden state for intrinsic Q networks.
            int_hidden_s = tuple(s.clone().to(device=self._device) for s in init_int_hidden_s)
            target_int_hidden_s = tuple(s.clone().to(device=self._device) for s in init_int_hidden_s)
        ...
        ext_retrace_loss, ext_priorities = self._calc_retrace_loss(learn_transitions, ext_q_t, target_ext_q_t.detach())
        int_retrace_loss, int_priorities = self._calc_retrace_loss(learn_transitions, int_q_t, target_int_q_t.detach())

        # Multiply loss by sampling weights, averaging over batch dimension
        loss = torch.mean((ext_retrace_loss + int_retrace_loss) * weights.detach())
    ...
    def _get_predicted_q_values(
        ...
        base.assert_rank_and_dtype(ext_hidden_state[0], 3, torch.float32)
        base.assert_rank_and_dtype(ext_hidden_state[1], 3, torch.float32)
        base.assert_batch_dimension(ext_hidden_state[0], self._batch_size, 1)
        base.assert_batch_dimension(ext_hidden_state[1], self._batch_size, 1)
        base.assert_rank_and_dtype(int_hidden_state[0], 3, torch.float32)
        base.assert_rank_and_dtype(int_hidden_state[1], 3, torch.float32)
        base.assert_batch_dimension(int_hidden_state[0], self._batch_size, 1)
        base.assert_batch_dimension(int_hidden_state[1], self._batch_size, 1)
    ...
    def _calc_rnd_loss(self, transitions: Agent57Transition) -> torch.Tensor:
    ...
    def _calc_embed_inverse_loss(self, transitions: Agent57Transition) -> torch.Tensor:
    ...
    def _burn_in_unroll_q_networks(
        ...
        _ext_hidden_s = tuple(s.clone().to(device=self._device) for s in ext_hidden_state)
        _int_hidden_s = tuple(s.clone().to(device=self._device) for s in int_hidden_state)
        _target_ext_hidden_s = tuple(s.clone().to(device=self._device) for s in ext_hidden_state)
        _target_int_hidden_s = tuple(s.clone().to(device=self._device) for s in int_hidden_state)

...
class MuZeroConfig:
    def __init__(self):
        ...
        self.train_on_gpu = torch.cuda.is_available()  or torch.backends.mps.is_available() # Train on GPU if available
...
class Game(AbstractGame):
    ...
    def __init__(self, seed=None):
        self.env = gym.make("Adventure-v4", render_mode="human")
        if seed is not None:
            self.env.seed(seed)
